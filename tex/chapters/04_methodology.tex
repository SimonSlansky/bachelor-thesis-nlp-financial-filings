...
Chapter~\ref{ch:data} are turned into the regressions that test the
two hypotheses set out in Chapter~\ref{ch:literature_review}.  Section
\ref{sec:design} gives an overview of the empirical strategy.
Sections~\ref{sec:variables}--\ref{sec:models} define the variables
and write out the three nested regression equations.
Section~\ref{sec:estimation} describes how the models are estimated
and how their standard errors are computed, and
Section~\ref{sec:hypothesis_tests} states the formal decision rules
used to evaluate H1 and H2.  Section~\ref{sec:robustness_plan}
previews the robustness checks reported in
...
The thesis estimates three nested panel regressions of post-filing
...
\Delta\text{Sentiment}_{i,t} \;-\; \lambda \cdot \Delta\text{ROA}_{i,t},
\label{eq:divergence}
\end{equation}
where $\Delta\text{ROA}_{i,t} = \text{ROA}_{i,t} - \text{ROA}_{i,t-1}$
and $\lambda$ is the slope coefficient of a within-firm regression of
$\Delta\text{Sentiment}$ on $\Delta\text{ROA}$ estimated on the
training sample.  The construction has a simple interpretation:
$\lambda \cdot \Delta\text{ROA}_{i,t}$ is the change in tone that
would, on average, accompany a change in profitability of size
$\Delta\text{ROA}_{i,t}$.  Subtracting it from the actual change in
tone gives the part of the narrative shift that is \emph{not}
explained by contemporaneous financial performance.  Large positive
values flag firms whose tone improved more than their fundamentals
warranted; large negative values flag the opposite.  As a robustness
check we also use a binary dummy that equals one when
