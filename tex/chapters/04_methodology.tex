...
financials and the textual features taken individually, and that the
sign of its effect is \emph{negative}: tone that moves more positively
than concurrent fundamentals justify operates as a costly, and therefore
credible, voluntary signal of management's private information about
future prospects, which lowers post-filing uncertainty.  H2 is supported
if the divergence coefficient is negative and statistically significant,
and if the divergence model improves on the text model in adjusted~$R^2$.
...
incremental adjusted~$R^2$ over the text model.  The directional
prediction follows the voluntary-disclosure reading of MD\&A: under
Securities Exchange Act \S\,18 and Rule~10b-5 a manager who writes
upbeat narrative against weak realised earnings bears a personal and
legal cost if the optimism is not borne out, so the disclosure is
costly and therefore credible.  Investors update toward management's
private signal and uncertainty about future cash flows falls,
implying a negative coefficient on Divergence.
...
\begin{enumerate}
  \item the divergence coefficient $\delta$ is negative and
        significant at the 5\% level, and
  \item the divergence model's adjusted~$R^2$ exceeds the text
        model's.
\end{enumerate}
A negative sign is required because the theoretical prediction is
directional.  When MD\&A tone moves more positively than
concurrent financial performance justifies, the disclosure is a costly
voluntary signal in the sense of the disclosure literature: management
can be sued under Rule~10b-5 if upbeat narrative is later contradicted
by outcomes, so the signal is credible only when management has private
information that future cash flows will exceed what current earnings
imply.  Investors who update toward this signal face less uncertainty
about future fundamentals, and post-filing volatility falls.  The
symmetric case---tone more negative than fundamentals warrant---signals
downgrade of private information and raises uncertainty in the same
direction.  Both arms of the prediction imply $\delta < 0$.
...
\begin{enumerate*}[label=(\roman*)]
  ...
  \item with post-filing return as the dependent variable instead of
        volatility;
  \item with the dependent variable measured over shorter post-filing
        windows (30, 90, and 180~calendar days), which lets us trace
        whether any textual signal decays as the disclosure ages;
